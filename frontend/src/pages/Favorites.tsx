...
const Favorites = () => {
  ...
  useEffect(() => {
    ...
    const loadFavorites = async () => {
      setLoading(true)
      try {
        const favoriteIds = await getFavoriteListingIds(firebaseUser.uid).catch((err) => {
  ...
  return (
    <div className="min-h-screen bg-[#f5f5f0] text-[#1f1f1f]">
      <Helmet>
        <title>Favorites | GC-Renting</title>
      </Helmet>
      <Header />
      ...
      <main className="mx-auto w-full max-w-6xl px-6 pb-16 pt-10">
        <div className="w-full space-y-6">
          <section className="rounded-3xl border border-black/5 bg-white p-6 shadow-sm">
            <h1 className="text-2xl font-semibold text-[#1f1f1f] md:text-3xl">Your favorites</h1>
            <p className="mt-2 text-sm text-gray-600">
              You have {favoriteCount} home{favoriteCount === 1 ? "" : "s"} saved from the Home and Rent pages.
            </p>
          ...
          {loading ? (
            ...
            <div className="rounded-3xl border border-black/5 bg-white p-8 text-center shadow-sm">
              You have not saved any properties yet. Tap the star icon on Home or Rent listings to add them here.
            </div>
          ) : (
