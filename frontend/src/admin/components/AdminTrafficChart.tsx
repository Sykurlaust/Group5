...
const buildOptions = (categories: string[]): ApexOptions => ({
  chart: {
...
const AdminTrafficChart = ({ title, categories, visits }: AdminTrafficChartProps) => {
  const series = [
    {
      name: "Visits",
      data: visits,
  ...
  return (
    <section className="flex h-[413px] w-full flex-col overflow-hidden rounded-[28px] border border-black/5 bg-white p-6 shadow-[0_10px_30px_rgba(15,23,42,0.06)]">
      ...
      <div className="mt-4 min-h-0 flex-1">
        <ReactApexChart height="100%" options={buildOptions(categories)} series={series} type="line" />
      </div>
    </section>
  )
}
